...
var secondStr = new Array( 12 ).fill( null ).map(function(t,i){
	var val = twobit( i *5 ); 
	return  '<li class="cell-2 js-second-cell" data-val="' + val + '">' + val + '</li>';
}).join('');
			...
			<div class="chose-all">\
				<div class="handle">\
					...
					<div class="cell-4"><a class="icon-up js-plus-minute"></a></div>\
                                        <div class="cell-2"></div>\
					<div class="cell-4"><a class="icon-up js-plus-second"></a></div>\
				</div>\
				<div class="text">\
					...
					<div class="cell-4"><a class="js-minute-show" title="选择分"></a></div>\
                                        <div class="cell-2">:</div>\
					<div class="cell-4"><a class="js-second-show" title="选择分"></a></div>\
				</div>\
				<div class="handle">\
					...
					<div class="cell-4"><a class="icon-down js-minus-minute"></a></div>\
                                        <div class="cell-2"></div>\
					<div class="cell-4"><a class="icon-down js-minus-second"></a></div>\
				</div>\
			</div>\
...
timepicker.choseSecond = content.find('.chose-second');
timepicker.hourShow = content.find('.js-hour-show');
timepicker.minuteShow = content.find('.js-minute-show');
timepicker.secondShow = content.find('.js-second-show');

// 更新时间
timepicker.update = function () {
	this.inputTarget.val( twobit( this.hour ) + ':' + twobit( this.minute ) +  ':' + twobit( this.second ) );
	this.minuteShow.text( twobit( this.minute ) );
	this.hourShow.text( twobit( this.hour ) );
        this.secondShow.text( twobit( this.second ) );
...
timepicker.bindEvent = function () {
	...
	// 分钟++
		...
		thisTimePicker.update();
        }).on('click','.js-minus-second',function() {
		var minute = thisTimePicker.second;
		if( minute <= 0 ){
			thisTimePicker.second = 59;
		} else {
			thisTimePicker.second--;
		}
		thisTimePicker.update();
                
        }).on('click','.js-plus-second',function() {
		var minute = thisTimePicker.second;
		if( minute >= 59 ){
			thisTimePicker.second = 0;
		} else {
			thisTimePicker.second++;
		}
	...
	// 选择分钟
        ...
        }).on('click','.js-second-cell',function () {
		thisTimePicker.second = +this.getAttribute('data-val');
		thisTimePicker.update();
		thisTimePicker.choseSecond.hide();
		thisTimePicker.choseAll.show();
		thisTimePicker.title.text('请选择');
	...
	thisTimePicker.minuteShow.on('click',function() {
        ...
        thisTimePicker.secondShow.on('click',function() {
		thisTimePicker.choseAll.hide();
		thisTimePicker.choseSecond.show();
		thisTimePicker.title.text('请选择分钟');
	});
};
